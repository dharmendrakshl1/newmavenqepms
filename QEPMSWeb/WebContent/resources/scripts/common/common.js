...
$("input.percentage").live("blur", function () {

   var originalVal = $.trim(this.getAttribute('value'));
   var currentVal =  $.trim(this.value);
   var x = Number(currentVal).toFixed(1);
   $(this).val(x).text(x);
   if ($(this).val()>100)
   {
...
$("input.amount").live("blur", function () {

   var originalVal = $.trim(this.getAttribute('value'));
   var currentVal =  $.trim(this.value);
   var x = Number(currentVal).toFixed(2);
   $(this).val(x).text(x);
   if (x=="NaN")
   {
	   $(this).val(originalVal);
...
if ($(".search input.amount").length != 0){
	$(".search input.amount").each(function() {
	 $(this).val("");
			
	$(".search input.amount").live("blur", function () 
	{	
	var originalVal = $.trim(this.getAttribute('value'));
	var currentVal =  $.trim(this.value);
	var x = Number(currentVal)
		if (x=="NaN" || x=="")
	   {$(this).val("");}
